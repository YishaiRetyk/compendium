...
**Plans**: 5 plans
Plans:
- [x] 12.2-01-PLAN.md — Wave 0: tests/phase-12.2/ test harness (run.sh + lib.sh + 11 test_*.sh covering 10 D-20 scenarios + D-05 contract)
- [x] 12.2-02-PLAN.md — Wave 1: bin/lint.sh --staged orthogonal flag + staged_new_pages() + STRICT_MODE dispatch + D-15 exemption ordering + LINT_VERSION 1.2.0
- [x] 12.2-03-PLAN.md — Wave 1: .githooks/pre-commit composition (sync-claude preserved; write-gate appended per D-07; D-08 footer line)
- [x] 12.2-04-PLAN.md — Wave 2: AGENTS.md §11.3 amendment (--staged row + Staged-mode rules subsection) + schema/AGENTS.template.md byte-equal mirror + docs/reference/ci.md "Local pre-commit write gate" section + CONTRIBUTING.md install reminder
