...
| 04-T1: Retro-classify source summaries | 04 | 2 | RPT-02, RPT-06 | T-19-04-A (self-citation unchanged) | Source summary |direct| count unchanged | file grep | `grep -c "r[0-9]*::" wiki-cloud/sources/src-2026-06-09-pdf-to-text-llm-ingestion-sota.md` → 12 | ❌ edits existing | ⬜ pending |
| 04-T2: Log + index + DR | 04 | 2 | RPT-05 | T-19-04-C (log format), T-19-04-D (DR fields) | UPDATE entries use compact dispatch form; DR has trigger_type: schema-update | full lint | `bash bin/lint.sh && bash bin/check-neutrality.sh` | ❌ creates new DR | ⬜ pending |
