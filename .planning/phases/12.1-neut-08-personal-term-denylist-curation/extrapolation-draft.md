...
### Class: ambiguous-not-vault-specific (49 terms)
Hyphenated/multi-word phrases that look distinctive but are widely used outside this vault (popular cog-bias terms, common idioms, generic time-spans).
...
- long-running, loss-aversion-based, low-stakes, meta-observation, non-recurring, one-to-one, paragraph-level, personal-development, personal-goals, pre-decision, pre-formed, pre-made, pre-mortem, pre-mortems, pre-reasoning, productivity-app, read-about, real-life, real-time, salary-negotiation
- self-diagnosis, single-decision, source-creation, stalled-project, sunk-cost, system-1-vs-system-2, two-week, utterance-level, week-long
...
## Summary
- Total candidates: 606 unique tokens (855 raw lines including duplicates across clusters)
- Kept: 1
- Rejected: 605 (generic-english=545 + ambiguous-not-vault-specific=49 + off-rubric=11)
- Per-cluster contribution to kept-list: agents-md-size-risk.md=0, personal-decision-patterns.md=1, personal-decision-journal.md=0
- Note: per RESEARCH.md A1, the agents-md-size-risk cluster (19 generic single-word tokens) yielded 0 promotions as expected.
- Note: Phase 7 already promoted 9 vault-coined hyphenated terms under category 'Personal-decision content archived from wiki/ (NEUT-08)'; Phase 12.1's expansion adds 1 term(s) — a gerund/verb-form completion of already-denied roots. Per MEDIUM #3 from REVIEWS.md, this kept-count is a sanity check; Plan 04 re-derives N from `git diff <phase-base>..HEAD -- .neutrality-denylist.txt | grep -cE '^\+[^#+]'` as the single source of truth.
