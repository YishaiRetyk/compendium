...
**Active milestone:** v1.4 Source Lifecycle (started 2026-07-03) — Phase 22 Repository Source Type (`REPO-01..06`: first *primary* instance of the Phase-19 extension contract; `#path:`/`#commit:` locators, curated-snapshot bundle convention, within-source epistemic split, lint/audit enforcement, real-repo validation) + Phase 23 External Source Drift Detection (`DRIFT-01..05`: promoted backlog 999.5; opt-in `--network` extension of the pre-plumbed `drift-external` lint subcategory — repository HEAD-vs-SHA, URL reachability, citation-registry link-rot; review-only). 11 requirements, 100% mapped. Live backlog (do not plan against superseded/promoted 999.1/999.2/999.4/999.5/999.7): 999.3 (template placeholder system, partially folded as the deferred Phase D/`WIZ`), 999.6 (observed GTD review patterns).
