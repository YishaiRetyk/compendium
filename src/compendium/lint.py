"""bin/lint.sh port (Phase 25 MIG-02) -- Wiki health-check CLI helper.

Byte-parity port of the frozen bash implementation (LINT_VERSION 1.12.0):
the bash arg-parse/dispatch becomes main(); the monolithic python3 heredoc
becomes _run_checks() with its logic lifted VERBATIM (indentation aside).
The wiki-page primitives (parse_frontmatter, mask_markdown, PROV_RE,
EPISTEMIC_INLINE_RE, WIKILINK_RE/PIPED_LINK_RE/BARE_LINK_RE,
EXCLUDE_FILES/EXCLUDE_DIRS) are IMPORTED from compendium.common.page --
the single canonical copy (PKG-02); this port retires the heredoc's inline
duplicates. Do not re-declare them here.

Zero LLM/API calls. Deterministic, file-based checks only (plus the opt-in
--network drift checks, which shell out to git/curl with identical argv).
"""
import os
import re
import subprocess
...
def _require_version_ok(required_version):
    """Version-pin check (D-28 minimum-version semantics, semver tuple
    ordering) -- lifted from the bin/lint.sh:209 heredoc."""
    try:
        running = tuple(map(int, LINT_VERSION.split('.')))
        required = tuple(map(int, required_version.split('.')))
    except ValueError:
        print(f"ERROR: bin/lint.sh --require-version expected semver X.Y.Z, got '{required_version}'", file=sys.stderr)
        return False
    if running < required:
        print(f"ERROR: bin/lint.sh --require-version {required_version} not satisfied. "
              f"Running version: {LINT_VERSION}. "
              f"Upgrade bin/lint.sh or lower the pin.", file=sys.stderr)
        return False
    return True


def _run_checks(wiki_dir, findings_file, dry_run, do_fix, category_filter,
                lint_format, ci_mode, skip_categories, strict_mode,
                network_mode, count_skips_mode, staged_mode, repo_root):
    """The bin/lint.sh python3 heredoc, verbatim (env-var reads replaced by
    parameters). Exits via sys.exit() on the CI/strict/JSON paths exactly as
    the heredoc does; main() converts SystemExit into the process exit code."""
    # --- Phase 9 CI-mode primitives (D-01..D-06) ---
    CI_MODE = ci_mode
    LINT_FORMAT = lint_format
    SKIP_CATEGORIES = set(filter(None, skip_categories.split(':')))

    # --- Phase 9 Plan 03 primitives (D-07..D-11, D-22) ---
    STRICT_MODE = strict_mode
    COUNT_SKIPS_MODE = count_skips_mode
...
def main(argv=None):
    """bin/lint.sh argument parsing + dispatch, ported line-for-line."""
    args = list(sys.argv[1:] if argv is None else argv)

    wiki_dir = os.environ.get('WIKI_ROOT') or 'wiki-cloud/'
    dry_run = 0
    fix = 0
    category = 'all'
    require_version = ''
    ...
    while i < n:
        ...
        if arg in ('--help', '-h'):
            ...
            print(LINT_VERSION)
            return 0
        elif arg == '--require-version':
            if n - i < 2:
                print("ERROR: --require-version requires a semver value (e.g., 1.1.0)", file=sys.stderr)
                return 1
            require_version = args[i + 1]
            i += 2
        elif arg == '--format':
            if n - i < 2:
            ...
            if args[i + 1] in ('text', 'json'):
                ...
                print(f"ERROR: --format must be 'text' or 'json', got '{args[i + 1]}'", file=sys.stderr)
                return 1
            i += 2
        elif arg == '--ci':
            ci_mode = 1
            i += 1
        elif arg == '--skip-category':
            if n - i < 2:
                print("ERROR: --skip-category requires a value", file=sys.stderr)
                return 1
            if not skip_categories:
                skip_categories = args[i + 1]
            else:
                skip_categories = f"{skip_categories}:{args[i + 1]}"
            i += 2
        elif arg == '--strict':
            strict_mode = 1
            i += 1
    ...
    # Version-pin check (D-28) -- runs before any filesystem validation.
    if require_version:
        if not _require_version_ok(require_version):
            return 1

    # Normalize wiki dir (strip trailing slash for consistency, re-add)
    if wiki_dir.endswith('/'):
        wiki_dir = wiki_dir[:-1]
    wiki_dir = wiki_dir + '/'
    ...
    # CI default skip: drift-external (D-02). Only applied when --ci and no
    # user --skip-category value was provided; an explicit --skip-category wins.
    if ci_mode == 1 and not skip_categories:
        skip_categories = 'drift-external'

    # Temp file for finding accumulation (bash: mktemp + EXIT trap).
    fd, findings_file = tempfile.mkstemp()
    os.close(fd)

    print(f"Linting {wiki_dir}...", file=sys.stderr)

    repo_root = os.environ.get('LINT_REPO_ROOT') or os.getcwd()
    ...
    try:
        try:
            _run_checks(
                ...
                count_skips_mode=(count_skips == 1),
                staged_mode=(staged_mode == 1),
                repo_root=repo_root,
            )
    ...
    if rc == 0:
        print("Lint complete.", file=sys.stderr)
    return rc
