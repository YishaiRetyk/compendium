# src/compendium/check_neutrality.py -- NEUT-06 + NEUT-08 gate.
# Phase 25 MIG-03 byte-parity port of bin/check-neutrality.sh: main() replicates the
# bash arg loop exactly (same error strings/streams/exit codes, usage text verbatim),
# exports the same CN_* env keys, then run() executes the bash python3-heredoc body
...
# Usage text extracted VERBATIM from the bin/check-neutrality.sh usage() heredoc.
USAGE = r"""Usage: bin/check-neutrality.sh [OPTIONS]
...
PUBLIC_PATHS_DEFAULT = [
    "AGENTS.md", "CLAUDE.md", "README.md", "PRIVACY.md",
    "docs", ".github", "wiki-cloud", "bin", "schema", ".claude/skills",
]

# ---------------------------------------------------------------------------
# Heredoc body (ported verbatim from the bin/check-neutrality.sh python3 heredoc).
# The CN_*-derived configuration globals are (re)read by run(); the constants and
# helper functions below are byte-identical to the heredoc's.
# ---------------------------------------------------------------------------
...
def run():
    """Execute the heredoc body: read CN_* env config, then dispatch.
    ...
    """
    global ROOT, SUGGEST, INCLUDE_GI, SHOW_STOPWORDS, FORMAT, DENYLIST, PUBLIC_PATHS
...
def main(argv=None):
    # Replicates the bin/check-neutrality.sh bash arg loop EXACTLY (same error
    ...
    while i < len(args):
        ...
        if a in ("--help", "-h"):
            ...
            root = args[i + 1]  # bash: unguarded "$2" (set -u aborts on a missing value)
            i += 2
        elif a == "--denylist":
            denylist = args[i + 1]  # bash: unguarded "$2"
            i += 2
        elif a == "--format":
            fmt = args[i + 1]  # bash: unguarded "$2"
            i += 2
        elif a == "--suggest-denylist":
            suggest_flag = 1
            i += 1
        elif a == "--include-gitignore":
            include_gitignore = 1
            i += 1
        elif a == "--show-stopwords":
            show_stopwords = 1
            i += 1
        elif a.startswith("-"):
    ...
    if not denylist:
        denylist = f"{root}/{DENYLIST_DEFAULT}"

    os.environ["CN_ROOT"] = root
    os.environ["CN_SUGGEST"] = str(suggest_flag)
    os.environ["CN_INCLUDE_GI"] = str(include_gitignore)
    os.environ["CN_SHOW_STOPWORDS"] = str(show_stopwords)
    os.environ["CN_FORMAT"] = fmt
    os.environ["CN_DENYLIST"] = denylist
    # Join PUBLIC_PATHS with ':' for env transport
    os.environ["CN_PUBLIC_PATHS"] = ":".join(PUBLIC_PATHS_DEFAULT)

    run()
    return 0


if __name__ == "__main__":          # enables `python3 -m compendium.check_neutrality`
