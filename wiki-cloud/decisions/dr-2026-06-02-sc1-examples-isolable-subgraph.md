...
type: decision
status: active
summary: "Phase 13.1 SC1 is renegotiated from 'graph not contaminated by examples/'
  to 'examples/ forms a visually isolable sub-graph,' because Obsidian's single Excluded-files
  mechanism cannot both keep the fixtures Dataview-indexed and hide them from the
  graph without mutating the committed .obsidian/."
