...
Ingested the AI Engineer conference talk "Agentic Search for Context Engineering" by Leonie Monigatti (Elastic), a ~63-minute video acquired as a transcript sub-case. Acquisition: audio downloaded and machine-transcribed with the `stt` CLI (faster-whisper large-v3, English at 99% confidence); automatic speaker diarization did NOT run (the pipeline's TorchCodec dependency was missing), so the committed single-file transcript carries no `SPEAKER:` labels — acceptable because the talk is single-speaker through 0:49:16 and the Q&A turns (audience, Monigatti, and her Elastic colleague Joe) are attributed inline from content. Raw transcript committed at `sources/2026/2026-07/2026-07-03-agentic-search-context-engineering.md` with a `## Description` section (official chapters + workshop repo + speaker links); a leading UTF-8 BOM was stripped so `#t` line-start locators resolve.

Created: source summary [[src-2026-07-03-agentic-search-context-engineering|Agentic Search for Context Engineering — Leonie Monigatti, Elastic]]; new concept [[agentic-search|Agentic Search]] (the RAG → agentic RAG → agentic-search arc, the multi-source context landscape, the shell/bash tool as a universal search interface, failure modes and tool/parameter design, and the low-floor/high-ceiling curated-stack recommendation); new entities [[elastic|Elastic]] and [[leonie-monigatti|Leonie Monigatti]].

Updated (append-then-synthesize): [[context-engineering|Context Engineering]] gained the practitioner "~80% agentic search" retrieval-centric reading complementing the ICM structuring framing; [[progressive-disclosure|Progressive Disclosure]] gained an independent agent-skill instance (just-in-time ES|QL syntax) plus the previously-implicit skill *eviction/offload* half of the discipline (Joe's file-store answer); [[agent-skills|Agent Skills]] gained the "skill as just-in-time tool documentation to fix parameter generation" use case and the official Elasticsearch skills note. Index updated with the three new pages and the four modified entries.

Classification: `source_type: transcript` (video sub-case). Privacy: cloud-safe (public technical talk, no personal data). Validity: clean single-speaker spoken word → page-level `sourced`, with claim-level `[epistemic:: tentative]` hedges across the STT failure surface — proper nouns and tool names the STT mangles (Jina rendered "Gina", GEPA rendered "GPA", LightOn `colgrep`, LlamaIndex `semtools`, OpenClaw), model versions (GPT 5.4 nano/mini), and figures (the "80%", `top_k` 3/10, 27 sessions). Contributor field omitted (single-author repo).

## [2026-07-03] ingest | openai/symphony — Symphony Service Specification repository snapshot

Ingested OpenAI's **Symphony** repository (`openai/symphony`) from the requested `SPEC.md` URL, classified `source_type: repository` — the wiki's second repository source after `src-2026-07-03-gsd-core-repo`. Acquisition: `git clone --depth 1` → metadata harvest (`git rev-parse HEAD` = `4cbe3a9699a73b862466c0b157ceca0c1985d6d7` on `main`; GitHub REST API for stars/license/language) → the full `SPEC.md` (2186 lines; 18 numbered sections + Appendix A) and the README embedded verbatim into the snapshot `source.md` so `#sec:` locators resolve offline, with `#commit:` project facts (Apache-2.0, Elixir, ~25,764 stars, `elixir/` reference impl) written into `## Snapshot Metadata`. All 20 `#sec:` locators plus the `#commit:` locator were pre-verified against the audit resolver before authoring.

Created: source summary [[src-2026-07-03-openai-symphony-spec|openai/symphony — Symphony Service Specification repository snapshot]]; new entities [[symphony|Symphony]] (the orchestration service — a long-running Linear-polling daemon, deterministic per-issue Codex workspaces, a single-authority orchestrator/state machine, a hot-reloaded `WORKFLOW.md` contract, filesystem safety invariants, the Linear GraphQL contract, and optional HTTP + SSH-worker extensions), [[openai|OpenAI]] (stub — publisher of Symphony/Codex, origin of the "harness engineering" framing), and [[codex|Codex]] (stub — OpenAI's coding agent, integrated via the app-server protocol with a thread/turn session model).

Updated (append-then-synthesize): [[spec-driven-development|Spec-Driven Development]] gained Symphony as a distribution-layer instance of SDD — OpenAI ships a language-agnostic spec you regenerate into code, the Elixir build labelled only an "experimental reference implementation" — via a new Key Fact, a Detail paragraph, an added source, and a re-synthesized TL;DR. Index updated with the three new entities, the new source, and the modified concept entry. The red link [[harness-engineering|Harness Engineering]] is left intentionally as a flagged knowledge gap.

Classification & epistemics: `source_type: repository`; privacy cloud-safe (public GitHub repo → `wiki-cloud/`). Within-source epistemic split applied — spec-behavior/architecture/project-fact claims are `sourced`/`direct` (`#sec:`/`#commit:`); self-descriptive claims (the "manage work, not agents" thesis, the "harness engineering" prerequisite) and the "engineering preview" maturity framing carry claim-level `[epistemic:: tentative]`; landscape contrasts (vs. the interactive dev-loop frameworks; the backpressure reading of "proof of work") are marked `[epistemic:: inferred]`. Entity pages set `epistemic_status: mixed` (spec-attested, not production-observed). Contributor field omitted (single-author repo).

## [2026-07-03] ingest | Building Great Agent Skills: The Missing Manual — Matt Pocock

Ingested the AI Engineer talk "Building Great Agent Skills: The Missing Manual" by Matt Pocock (spoken title "The Missing Manual, How to Write Great Skills"), a ~20-minute video acquired as a transcript sub-case. Acquisition: audio downloaded and machine-transcribed with the `stt` CLI (faster-whisper large-v3, English); automatic speaker diarization did NOT run (the pipeline's TorchCodec dependency was missing), so the committed single-file transcript carries no `SPEAKER:` labels — acceptable because the talk is single-speaker throughout. Raw transcript committed at `sources/2026/2026-07/2026-07-03-building-great-agent-skills.md` with a `## Description` section (official description, chapters, skills-repo + featured-skill + newsletter links).

Created: source summary [[src-2026-07-03-building-great-agent-skills|Building Great Agent Skills: The Missing Manual — Matt Pocock]]; new concept [[skill-checklist|Skill Checklist]] (the four-part authoring/audit rubric — Trigger, Structure, Steering, Pruning — plus the user-invoked vs model-invoked distinction, context pointers, leading words, legwork-per-step, sediment, and no-ops); new entity [[matt-pocock|Matt Pocock]].

Updated (append-then-synthesize): [[agent-skills|Agent Skills]] gained the practitioner rubric that complements Anthropic's official authoring guidance — the trigger design axis (user-invoked vs model-invoked, context load vs cognitive load) the docs leave implicit, plus the steps+reference / minimal-`SKILL.md` / leading-words / pruning heuristics; [[progressive-disclosure|Progressive Disclosure]] gained the authoring-side restatement (the "context pointer" unit and the keep-`SKILL.md`-small / hide-branching-reference rules); [[superpowers|Superpowers]] gained Matt Pocock's cross-observation characterizing it as "primarily model-invoked." Index updated with the two new pages, the new source, and the modified entries.

Classification: `source_type: transcript` (video sub-case). Privacy: cloud-safe (public technical talk, no personal data). Validity: clean single-speaker studio narration → page-level `sourced`, with claim-level `[epistemic:: tentative]` hedges across the STT failure surface — the speaker name (rendered "matt pocott" once), his example skill names (2PRD, grill me, grill with docs, domain modeling, code base design), coined terms ("leading words" glossed with a literary aside the STT renders "light vert", most likely *Leitwort*), the `disable model invocation: true` flag, and illustrative numbers. The repo/skill/newsletter URLs are taken from the official video description, not the STT. Contributor field omitted (single-author repo).

## [2026-07-03] ingest | The Future Is Domain-Specific Agents — Justin Schroeder, StandardAgents

Ingested the AI Engineer talk "The Future Is Domain-Specific Agents" by Justin Schroeder (StandardAgents), a ~31-minute video acquired as a transcript sub-case. Acquisition: audio downloaded and machine-transcribed with the `stt` CLI (faster-whisper large-v3, English, user-specified); automatic speaker diarization did NOT run (the pipeline's TorchCodec dependency was missing), so the committed single-file transcript carries no `SPEAKER:` labels — acceptable because the talk is single-speaker throughout. Raw transcript committed at `sources/2026/2026-07/2026-07-03-domain-specific-agents.md` with a `## Description` section (official description + speaker links). The speaker's name/handle/company/projects are taken from the official video description (via `yt-dlp`), not the STT, which renders "Schroeder" as "Schrader," "@jpschroeder" as "JPSchrader," and "ArrowJS" as "Aero.js."

Created: source summary [[src-2026-07-03-domain-specific-agents|The Future Is Domain-Specific Agents — Justin Schroeder, StandardAgents]]; new concept [[domain-specific-agents|Domain-Specific Agents]] (composition over inheritance for agents — many small isolated agents under a coordinator communicating in plain English; the four claimed benefits of token efficiency / cheap small models / strict capability limits / parallel scaling; the ideal-agent anatomy of functions-prompts-agent-as-tool, hooks, agent rules, sandboxed filesystem + code execution; recursive sub-agents; and the 2026–2027 multi-agent-orchestration predictions); new entities [[justin-schroeder|Justin Schroeder]] and [[standardagents|StandardAgents]] (the latter graded `epistemic_status: tentative` since it is a stealth pre-announcement company documented only from this one talk).

Updated (append-then-synthesize): [[agent-skills|Agent Skills]] gained the dissenting "skills as context inflation" view — a skill is fundamentally a markdown documentation file, too many skills degrade the agent, and piling skills/tools/MCP onto one agent is "inheritance" whose alternative is composition into small agents; [[subagents|Subagents]] gained the architecture-level generalization of a sub-agent into an *agent-as-a-tool* composition primitive with recursive nesting, beyond Claude Code's `.claude/agents/*.md`. Index updated with the two new entities, the new concept, and the new source; the two modified pages picked up the new source in their frontmatter and Sources lists.

Classification: `source_type: transcript` (video sub-case). Privacy: cloud-safe (public conference talk, no personal data). Validity: clean single-speaker studio narration → page-level `sourced`, with claim-level `[epistemic:: tentative]` hedges across the STT failure surface — proper/product/model names as-heard (StandardAgents, dmux, ArrowJS, OpenClaw, Hermes, Vercel "Eve", DeepSeek V4 Flash, Fable 5, GLM 5.2, "nano banana"), the speaker's own internal/unpublished figures (>80% token efficiency, "137× cheaper" DeepSeek-vs-Fable, 2026 token-cost +29–30%/+76%, "10,000 turns", "45 MB of GDPR context"), and his forward-looking predictions (2026 rise of domain-specific agents; 2027 "year of multi-agent orchestration"; the token-cost reversal). Description-only facts (full project list, GitHub/LinkedIn) cited with a `#sec:description` locator into the raw source. Contributor field omitted (single-author repo).

## [2026-07-03] ingest | Beyond the Prompt: Goodbye Slop, Welcome Determinism — David Khourshid

Ingested the "Beyond the Prompt" conference talk "Goodbye Slop, Welcome Determinism" by David Khourshid (creator of XState, founder of Stately.ai; AG Grid channel), a ~30-minute video acquired as a transcript sub-case. Acquisition: audio downloaded and machine-transcribed with the `stt` CLI (faster-whisper large-v3, English, user-specified); automatic speaker diarization did NOT run (the pipeline's TorchCodec dependency was missing), so the committed single-file transcript carries no `SPEAKER:` labels — acceptable because the talk is single-speaker throughout. Raw transcript committed at `sources/2026/2026-07/2026-07-03-goodbye-slop-welcome-determinism.md` with a `## Description` section (official description + speaker bio + X handle). The speaker's name/handle are taken from the official video description (via `yt-dlp`), not the STT, which renders "determinism" as "Terminism," "David K Piano"/@DavidKPiano as "David K. Piena"/"Pino," "XState" as "Xstate," "Claude" as "clod," "useState" as "use dates," and "Gary Bernhardt" as "Cary Bernhardt."

Created: source summary [[src-2026-07-03-goodbye-slop-welcome-determinism|Beyond the Prompt: Goodbye Slop, Welcome Determinism — David Khourshid]]; new concept [[deterministic-core-agentic-shell|Deterministic Core, Agentic Shell]] (the architectural inversion — determinism at the core, non-determinism at the edges; write programs that call LLMs, not LLMs that call programs; the "slop code = code without a reliable model" definition; unstructured delegation as root cause; the explicit-model remedy; the failure modes of one-shotting / prose control flow / agents-as-the-system / entanglement of concerns / bigger-context-windows; and the Gary Bernhardt / Ken Wheeler lineage); new entities [[david-khourshid|David Khourshid]] and [[xstate|XState]] (the latter covering Stately.ai and statecharts).

Updated (append-then-synthesize): [[comprehension-debt|Comprehension Debt]] gained the parallel "slop code" framing reached from the modeling side — Khourshid's "code without a reliable model" restates the same failure as comprehension debt, with unstructured delegation as the diagnosed cause and the explicit model as the remedy; a Key Fact, a "Parallel framing" detail subsection, the new source, and a Related-Pages cross-link were added. Index updated with the two new entities, the new concept, and the new source.

Classification: `source_type: transcript` (video sub-case). Privacy: cloud-safe (public conference talk, no personal data). Validity: clean single-speaker studio narration → page-level `sourced`, with claim-level `[epistemic:: tentative]` hedges across the STT failure surface — proper names as-heard (Ken Wheeler, Gary Bernhardt, Matt Pocock), technical terms (DFA/NFA, Gherkin/Cucumber, BPMN, useState, TanStack), product/version names (XState v6, XState Store v4, the "stately.sketch.ai" editor URL), and the speaker's own rhetorical figures (~90% of code is not behavioral intent, 15–20 unfinished side projects, "10× speeds"). The "deterministic core, agentic shell" article and the person who "called him out" are unnamed in the transcript. Contributor field omitted (single-author repo).

## [2026-07-03] ingest | Software Engineering at the Tipping Point — Adam Bender, Google I/O 2026

Ingested the Google I/O 2026 keynote "Software Engineering at the Tipping Point" by Adam Bender (Google for Developers channel), a ~40-minute video acquired as a transcript sub-case. Acquisition: audio downloaded and machine-transcribed with the `stt` CLI (faster-whisper large-v3, English, user-specified); automatic speaker diarization did NOT run (the pipeline's TorchCodec dependency was missing), so the committed single-file transcript carries no `SPEAKER:` labels — acceptable because the talk is single-speaker throughout. Raw transcript committed at `sources/2026/2026-07/2026-07-03-software-engineering-tipping-point.md` with a `## Description` section (official description, event, speaker, links). The audio is clean studio conference narration and proper nouns transcribe accurately (Conway's Law, Jeff Atwood, Jevons paradox, the "Flamingo book"); the one canonical-spelling note is that the STT renders **DORA** (Google's DevOps Research and Assessment group) as "Dora."

Created: source summary [[src-2026-07-03-software-engineering-tipping-point|Software Engineering at the Tipping Point — Adam Bender, Google I/O 2026]]; new concepts [[software-ecology|Software Ecology]] ("the holistic study of the socio-technical ecosystems that produce software" — the system → ecosystem → complex adaptive system → emergence → socio-technical vocabulary, Conway's Law, and trade-offs-reveal-values), [[shared-fate|Shared Fate]] (the coupling-degree design choice: the monorepo "ten lines patch ten billion" superpower vs. cascading-failure hazard), [[10x-moment|The 10x Moment]] (the 10x–100x code-production surge that non-uniformly stresses every downstream node; "generating code 10x faster ≠ engineering 10x faster"; the quadratic test blow-up and node-by-node tour), and [[ai-as-amplifier|AI as Amplifier]] ("amplification is a magnitude, not a direction," credited to DORA; fundamentals set the direction); new entities [[adam-bender|Adam Bender]] and [[google|Google]] (the latter scoped to its developer ecosystem — monorepo, trunk-based development, shared fate, large-scale changes, engineering culture).

Updated (append-then-synthesize): [[systems-thinking|Systems Thinking]] gained Bender's extension of the skill from a single code base to the whole *developer ecosystem* — the CAS/emergence vocabulary, the "why / what if" two-question drill, the tools-of-systems-analysis list, and the framing that two independent sources (Hack; Bender) converge on systems thinking as the core AI-era skill (re-synthesized TL;DR, four new Key Facts, a new Detail subsection, added source and cross-links); [[comprehension-debt|Comprehension Debt]] gained Bender's positive framing "intellectual control" ("can humans reason about this thing in front of them?") plus the precise 10x mechanism by which the code-review bottleneck mass-produces the debt ("who's paying attention to the code base as it evolves? No one"). Index updated with the two new entities, four new concepts, the new source, and the two modified concept entries.

Classification: `source_type: transcript` (video sub-case). Privacy: cloud-safe (public Google I/O keynote, no personal data). Validity: clean single-speaker studio narration → page-level `sourced`, with claim-level `[epistemic:: tentative]` hedges across the STT failure surface — proper nouns/named entities (Google, Conway's Law, Jeff Atwood, DORA, Jevons paradox, Android, Chrome, the "Flamingo book"), coined/technical terms (software ecology, shared fate, large-scale changes, conjunction of Booleans, intellectual control), and the speaker's own rhetorical/illustrative figures (10x–100x, "10–15x in 18 months," "25 years," "15 years," "ten lines patch ten billion," the quadratic "100×–1,000× tests," "40% growth," the "2030 feels like 2001" forecast). The "Flamingo book" is identified as *Software Engineering at Google* as an `[epistemic:: inferred]` note (the talk gives only the nickname). Contributor field omitted (single-author repo).
