...
type: concept
status: active
summary: "A deliberately minimal autonomous-coding pattern by Geoffrey Huntley: a
  bash `while` loop re-feeds a fixed PROMPT.md into a CLI agent each iteration, with
  IMPLEMENTATION_PLAN.md on disk as the only cross-iteration shared state."
