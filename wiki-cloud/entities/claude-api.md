...
type: entity
status: active
summary: "Anthropic's HTTP API surface (Messages API + Skills API). Supports both
  pre-built Anthropic-managed Agent Skills (pptx/xlsx/docx/pdf) and custom uploaded
  Skills via /v1/skills, invoked by passing skill_id in the Messages API container
  parameter alongside the code_execution_20250825 tool. Three required betas; no network
  access; pre-installed packages only."
