...
write_page "$REPO" "wiki-cloud/sources/src-pa.md" <<'EOF'
---
id: src-pa
title: "PA"
type: source
...
write_page "$REPO" "wiki-cloud/concepts/pp.md" <<'EOF'
---
id: pp
title: "PP"
type: concept
...
set +e
out="$(cd "$REPO" && AUDIT_REPO_ROOT="$REPO" invoke_tool_compat audit-claims --emit-worklist --format json 2>/dev/null)"
rc=$?
set -e
