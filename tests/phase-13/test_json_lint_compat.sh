...
write_page "$REPO" "wiki/sources/src-j.md" <<'EOF'
---
id: src-j
title: "J"
type: source
...
Body text.
EOF
write_page "$REPO" "wiki/concepts/j.md" <<'EOF'
---
id: j
title: "J"
type: concept
...
set +e
out="$(cd "$REPO" && AUDIT_REPO_ROOT="$REPO" bash "$REPO_ROOT/bin/audit-claims.sh" --since "$SEED" --format json 2>/dev/null)"
rc=$?
set -e
