...
# Assert ordering of the ACTUAL gate INVOCATIONS, not comment mentions (REVIEW WR-04).
# Match the `bash bin/<tool>.sh ... --check`/invocation lines specifically -- the `bash bin/`
# prefix excludes the explanatory comment blocks that also name these tools, so this verifies
# the order the three gates really execute rather than incidentally passing on comment order.
sync_line=$(grep -nE '^\s*if ! bash bin/sync-claude\.sh' "$hook" | head -1 | cut -d: -f1)
gen_line=$(grep -nE '^\s*if ! bash bin/gen-skills\.sh' "$hook" | head -1 | cut -d: -f1)
lint_line=$(grep -nE '^\s*if ! bash bin/lint\.sh' "$hook" | head -1 | cut -d: -f1)
