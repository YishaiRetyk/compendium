...
# section headings, critical frontmatter values, and an inline wiki-cloud/index.md append snippet
...
if ! grep -q 'cat > "wiki-cloud/decisions/dr-${TODAY}-initial-setup.md" <<EOF' "$DOC"; then
    echo "ASSERT FAIL: manual-setup.md Section 8 must contain the decision-record heredoc 'cat > \"wiki-cloud/decisions/dr-\${TODAY}-initial-setup.md\" <<EOF' (review concern #4)" >&2
    exit 1
fi
...
# Inline wiki-cloud/index.md wikilink must match Plan 03's update_index_md() output exactly
if ! grep -qF '[[dr-${TODAY}-initial-setup|Initial Wizard Setup -- ${PRIMARY_DOMAIN}]]' "$DOC"; then
    echo "ASSERT FAIL: Section 8 must contain the exact wiki-cloud/index.md wikilink format '[[dr-\${TODAY}-initial-setup|Initial Wizard Setup -- \${PRIMARY_DOMAIN}]]' (byte-parity with Plan 03)" >&2
    exit 1
fi

# Manual track must NOT defer decision-record creation to the wizard
if grep -qE '(run the wizard|invoke the wizard|bin/init-wizard\.sh).*to (generate|produce|create).*(decision|wiki-cloud/decisions)' "$DOC"; then
    ...
    grep -nE '(run the wizard|invoke the wizard|bin/init-wizard\.sh).*to (generate|produce|create).*(decision|wiki-cloud/decisions)' "$DOC" >&2
    exit 1
fi

echo "PASS: manual-setup.md Section 8 contains self-contained decision-record heredoc with 7 required headings + inline wiki-cloud/index.md append snippet (review concern #4)"
exit 0
