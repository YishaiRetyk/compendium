...
# wiki-cloud/index.md is updated with a `## Decisions` subsection + wikilink entry
...
echo "TEST: wiki-cloud/index.md update_index_md() 3-guardrail contract (Open Q1, review concern #2)"
...
assert_grep '^## Decisions$' "$WORK/wiki-cloud/index.md" "Decisions heading must be created"
assert_grep '\[\[dr-2026-04-16-initial-setup\|Initial Wizard Setup -- personal-knowledge\]\]' \
    "$WORK/wiki-cloud/index.md" "Decisions wikilink entry must be present"

header_count="$(grep -c '^## Decisions$' "$WORK/wiki-cloud/index.md")"
...
header_count2="$(grep -c '^## Decisions$' "$WORK2/wiki-cloud/index.md")"
assert_eq 1 "$header_count2" "idempotent re-run must keep exactly one '## Decisions' heading"

entry_count="$(grep -c 'dr-2026-04-16-initial-setup' "$WORK2/wiki-cloud/index.md")"
