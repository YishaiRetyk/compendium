...
| `tests/phase-15/test_neutrality_leak_source_rekey.sh:15+` | greps the `check-neutrality.sh` SOURCE (via a path variable) for PUBLIC_PATHS/denylist coverage of specific paths | impl-asserting | **defer-to-MIG-03** | Source-read through variable indirection (found during the Plan-05 seam routing — the mechanical discovery cannot see `grep … "$VAR"`). The MIG-03 checkers port must rewrite these to behavior probes (run the checker against a fixture containing the covered path). |
| `tests/phase-18/test_neutrality_covers_skills.sh:6` | greps `check-neutrality.sh` SOURCE for `.claude/skills` in PUBLIC_PATHS | impl-asserting | **defer-to-MIG-03** | Same variable-indirection class as above. |
