...
bash "$LINT" --require-version 1.1.0 --dry-run >/dev/null 2>&1 \
    || { echo "FAIL: --require-version 1.1.0 should pass when LINT_VERSION=1.4.0" >&2; exit 1; }
...
grep -q "1.5.0" /tmp/require-err \
    || { echo "FAIL: stderr missing '1.5.0' token" >&2; exit 1; }
grep -q "1.4.0" /tmp/require-err \
    || { echo "FAIL: stderr missing '1.4.0' running version" >&2; exit 1; }

# Case 4: semver tuple ordering (1.10.0 > 1.4.0 -- bash string compare would get this wrong)
if bash "$LINT" --require-version 1.10.0 --dry-run >/dev/null 2>&1; then
    echo "FAIL: --require-version 1.10.0 should fail when LINT_VERSION=1.4.0 (semver tuple, not string)" >&2
    exit 1
fi
