...
3. **Classify** (Pipeline Pass 0): Determine source type -- article, paper, transcript, journal entry, data file, image-heavy, or research-report. See `schema/reference/source-types.md` for type definitions and the extension decision rule.
   ...
   - **PDF sources:** PDFs are a format-orthogonal acquisition sub-case, NOT a source_type. Classify the content to its parent type (article / paper / data / ...) as normal; then layer the PDF convention (extraction frontmatter fields, `#p` locators, tiered epistemic policy). See `schema/reference/pdf-ingestion.md`.
   - **Video / YouTube sources:** a video is a sub-case of `transcript`, NOT a new source_type. Classify to `transcript` as normal; then layer the video convention (the five frontmatter fields, `#t` locators, tiered epistemic policy, link-rot drift stance). See `schema/reference/video-ingestion.md`.
   - **Repository sources:** a code repository IS a first-class `source_type: repository` (NOT a sub-case — no parent type exists for a versioned file tree). Classify directly; the raw source is a curated snapshot bundle with an `## Excerpts` registry, claims use `#path:`/`#commit:` locators with `support_type: direct`, and README self-descriptions get claim-level `tentative` hedging. See `schema/reference/repository-ingestion.md`.
...
| repository | Atomic claims, `support_type: direct` | Anchor code claims `#path:` (quote the code in the snapshot's `## Excerpts` registry), snapshot-commit claims `#commit:`, docs prose `#sec:`. Hedge README self-descriptions claim-level `[epistemic:: tentative]` (see `schema/reference/repository-ingestion.md`). |
