...
| `--ci` | Apply severity-remap dispatch table: `yaml`/`orphan`/`crossref`/`provenance`/`linkres` -> `error`; `routing` forward findings (dangling refs, `§N` pattern violations) -> `error`, with two inverse exceptions that stay non-gating — `ORPHAN:`-prefixed routing -> `warning` and `AUDIT:`-prefixed routing (WF-08 drift) -> `info`; `stale`/`gap`/`contradiction`/`contradiction-sync`/`drift`/`contributor` -> `warning`; `autofix`/`skip-count` -> `info`. Default-skip `drift-external` category. Exit 1 iff any post-remap finding has severity `error`. |
...
3. **Provenance validation:** Verify all `[prov:]` references resolve to known source IDs in `wiki-cloud/sources/` or `wiki-local/sources/`. Verify locator syntax (see `schema/reference/provenance.md`). Severity: error for broken refs. **Tier-scope limit:** `bin/lint.sh` walks one wiki directory per run (default `wiki-cloud/`), so the derived-never-direct gate (D-08) covers cloud-tier pages only — a `wiki-local/` page citing a cloud research-report is not checked in default runs (and a `wiki-local/`-targeted run builds its source registry from local source pages only, producing false broken-ref errors for cloud sources). Local-tier coverage is a known gap, not a guarantee.
...
**Categories** (valid values for `--category` filter): `orphan`, `crossref`, `stale`, `contradiction`, `gap`, `provenance`, `yaml`, `drift`, `duplicate`, `contributor`, `brownfield`, `linkres`, `routing`.
