...
**Hollow-audit tripwire.** After resolution, the script computes the locator-resolution ratio over the claims that attempted passage extraction (deliberate `skipped-privacy`/`skipped-nontext` skips are excluded from the base). When fewer than 50% resolve (minimum 5 attempts), it prints a prominent stderr WARNING and appends a warning-severity finding to the report: a run where most locators fail is checking almost nothing, and the per-claim `insufficient-locator` verdicts alone proved too quiet to surface that. The tripwire is warning-only — exit code and the review-only contract are unchanged.
