# openai/symphony — repository snapshot
...
- Stars at snapshot: 25,764
- Homepage: https://openai.com/index/open-source-codex-orchestration-symphony/
- Repository created (GitHub): 2026-02-26; last push observed: 2026-06-09
- Retrieved: 2026-07-03 (UTC); ingested 2026-07-03
- Acquisition: `git clone --depth 1` → metadata harvest (`git rev-parse HEAD`, GitHub REST API for stars/license/language) → README + SPEC embedded verbatim below.
- Tree stats at snapshot: top-level `SPEC.md` (the language-agnostic service specification — 18 numbered sections plus Appendix A), `README.md`, `LICENSE` (Apache-2.0), `NOTICE`, a `docs/` directory, and an `elixir/` reference implementation. The tracked tree is dominated by Elixir sources (37 `.ex` + 22 `.exs` files) alongside Markdown docs (21 `.md`); the Elixir reference implementation is the only implementation shipped in-repo.

## README

<!-- readme H1 demoted at snapshot: Symphony -->
<!-- curator note: demo-video poster-image embed trimmed; the Vimeo demo link is retained inline in the prose paragraph below. -->
...
<!-- Below: the repository's SPEC.md (openai/symphony@4cbe3a9), embedded verbatim as the primary curated document so #sec: locators resolve against the snapshot offline. This SPEC is the file this ingest was requested against. -->
